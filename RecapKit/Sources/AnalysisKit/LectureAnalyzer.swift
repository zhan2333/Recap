...
public struct LectureAnalyzer {
    ...
    转写稿的已知缺陷，处理时注意：
    - 来自 whisper 语音识别，专业术语常有同音错字（如"土力学"写成"图的学"、"固结"写成"固解"），仅纠正课堂上下文能够确认的识别或理解错误；数字、公式、否定词不确定时保留待核实，不凭专业常识补齐；
    ...
    提取规则：
    - 中英文都以视频课堂及本次转写为主，实际讲授决定内容范围、顺序、案例和考试重点；参考材料只辅助校正转写理解，不新增老师没讲的主题、考点或必背要求。只收到转写时不能声称已回听视频。
    - 老师明确说法与教材或常识不同时，不自动判为识别错误或静默替换；保留课堂说法及适用条件，将无法确认的差异标为待核实，不包装成已核实结论。
    - 体现老师传递的轻重：保留明确掌握要求、重点讲解、典型课堂案例、易错提醒，以及略讲/了解即可等信号；同类条目可按有依据的重要程度排序，不把全部主题平均列为重点。教材篇幅、一般知识的重要性、讲授时长或重复次数都不能单独决定考试强度。
